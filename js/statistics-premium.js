...
(function(){
  ...
  const recordedAt = t => {
    try {
      ...
      if(n.startsWith('[[ORDER_ITEMS]]')) { const p=JSON.parse(n.slice('[[ORDER_ITEMS]]'.length)); if(p?.recorded_at) return p.recorded_at; }
    } catch {}
    return t?.created_at || t?.updated_at || '';
  };
  const fmtDateTime = t => { const raw=recordedAt(t); const d=raw?new Date(raw):new Date((dstr(t?.date)||'')+'T12:00:00'); if(Number.isNaN(d.getTime())) return fmtDate(t?.date); return new Intl.DateTimeFormat('vi-VN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(d); };
  const parseItems = t => {
  ...
  function orderRows(income){
    const ordered=[...income].sort((a,b)=>new Date(recordedAt(b)||((dstr(b?.date)||'')+'T12:00:00')).getTime()-new Date(recordedAt(a)||((dstr(a?.date)||'')+'T12:00:00')).getTime());
    return ordered.map((t,i)=>{
      ...
      return `<article class="sp-order-card">
        <div class="sp-order-head"><div><div class="sp-order-title">${s.icon} Đơn #${i+1} <span class="sp-source ${s.cls}">${s.name}</span></div><div class="sp-order-meta">🕒 ${fmtDateTime(t)}${t.note && typeof window.getTransactionDisplayNote==='function' && window.getTransactionDisplayNote(t)?' · '+esc(window.getTransactionDisplayNote(t)):''}</div></div><div class="sp-net"><span>THỰC NHẬN</span><b class="${net>=0?'pos':'neg'}">${fmt(net)}</b></div></div>
  ...
  function renderShell(){
    ...
    page.innerHTML=`<div class="sp-wrap">
      <section class="sp-topbar card">
        <div class="sp-title-block">
          <span class="sp-kicker">TỔNG QUAN TÀI CHÍNH</span>
          <h2>Thống kê</h2>
          <p>Nhìn nhanh doanh thu, vốn, phí và lợi nhuận; xuống dưới để đối soát từng đơn.</p>
        </div>
        <div class="sp-topbar-status"><span class="sp-live-dot"></span><span>Dữ liệu từ giao dịch đã lưu</span></div>
      </section>

      <section class="sp-control card">
        <div class="sp-control-top">
          <div><span class="sp-kicker">KHOẢNG THỜI GIAN</span><h3>Chọn kỳ thống kê</h3><p>Xem theo ngày, tuần hoặc tháng.</p></div>
          <button id="spToday" type="button" class="sp-today">Hôm nay</button>
        </div>
        <div class="sp-tabs"><button type="button" data-period="day">Ngày</button><button type="button" data-period="week">Tuần</button><button type="button" data-period="month">Tháng</button></div>
        <div class="sp-date-row">
          <button id="spPrev" type="button" class="sp-nav-btn" aria-label="Kỳ trước">‹</button>
          <div class="sp-period-center"><strong id="spPeriodLabel" class="sp-period-label">Hôm nay</strong><small>Khoảng thời gian đang xem</small></div>
          <button id="spNext" type="button" class="sp-nav-btn" aria-label="Kỳ sau">›</button>
          <label class="sp-date-field"><span>Ngày tham chiếu</span><input id="spDate" type="date"></label>
        </div>
      </section>
      ...
      <section class="sp-kpi-grid">
        ...
        <article class="sp-kpi fee"><div class="sp-kpi-icon">%</div><div><span>PHÍ NỀN TẢNG</span><strong id="spFee">0 ₫</strong><small id="spFeeRate">0 đơn có phí app</small></div></article>
      ...
      <section class="sp-reading card">
        <div class="sp-reading-head"><div><span class="sp-kicker">ĐỌC NHANH</span><h3>Kỳ này đang lời hay lỗ?</h3></div><div class="sp-reading-formula">Doanh thu − Giá vốn − Phí app − Chi phí = Lợi nhuận thực</div></div>
        <div class="sp-story-grid">
      ...
      <section class="sp-two-col">
        <article class="sp-section card"><div class="sp-section-head"><div><span>DIỄN BIẾN</span><h3>Doanh thu & chi phí</h3><p>Nhìn ngày nào thu tốt và ngày nào chi nhiều.</p></div><div class="sp-legend"><span><i class="rev"></i>Thu</span><span><i class="exp"></i>Chi</span></div></div><div id="spChart" class="sp-chart"></div></article>
        <article class="sp-section card"><div class="sp-section-head"><div><span>CƠ CẤU</span><h3>Doanh thu đi đâu?</h3><p>Tỷ trọng vốn, phí, chi phí và lợi nhuận.</p></div></div><div id="spStructure"></div></article>
      </section>

      <section class="sp-section card"><div class="sp-section-head"><div><span>NỀN TẢNG</span><h3>Hiệu quả từng nguồn đơn</h3><p>Mỗi nguồn gồm doanh thu, vốn, phí và số tiền thực nhận.</p></div></div><div id="spSources"></div></section>

      <section class="sp-section card"><div class="sp-section-head"><div><span>MÓN BÁN</span><h3>Món bán tốt nhất</h3><p>Số lượng, doanh thu, vốn và lợi nhuận ước tính theo món.</p></div></div><div id="spDishes"></div></section>

      <section class="sp-section card"><div class="sp-section-head"><div><span>DOANH THU</span><h3>Chi tiết từng đơn</h3><p>Mỗi đơn: tổng tiền → giá vốn → chiết khấu app → tiền thực nhận.</p></div><strong class="sp-section-count" id="spOrderCountLabel"></strong></div><div id="spOrdersList"></div></section>

      <section class="sp-two-col">
        <article class="sp-section card"><div class="sp-section-head"><div><span>CHI PHÍ</span><h3>Chi phí theo danh mục</h3><p>Danh mục nào đang tiêu nhiều nhất.</p></div></div><div id="spExpenseCats"></div></article>
        <article class="sp-section card"><div class="sp-section-head"><div><span>ĐỐI SOÁT</span><h3>Danh sách khoản chi</h3><p>Kiểm tra từng khoản chi theo ngày.</p></div></div><div id="spExpenseList"></div></article>
      </section>

      <section class="sp-bottom-note card"><div class="sp-bottom-icon">💡</div><div><b>Cách đọc lợi nhuận thực</b><span>Đơn ShopeeFood/GrabFood trừ phí app một lần cho cả đơn. Đơn Ngoài sàn không trừ phí app.</span></div></section>
    </div>`;
