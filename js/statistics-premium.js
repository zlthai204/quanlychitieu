...
(function(){
  ...
  const getRange = () => {
    const p=window.AppState?.statisticsPeriod || 'day';
    const base=window.AppState?.statisticsDate instanceof Date ? new Date(window.AppState.statisticsDate) : new Date();
  ...
  const transactionsInRange = () => {
    ...
    return (window.AppState?.transactions||[]).filter(t=>{const d=dstr(t?.date); return d && d>=start && d<=end;});
  };
  const setText=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};

  function ensureState(){
    if(!window.AppState) window.AppState={};
    if(!['day','week','month'].includes(window.AppState.statisticsPeriod)) window.AppState.statisticsPeriod='day';
    if(!(window.AppState.statisticsDate instanceof Date) || isNaN(window.AppState.statisticsDate)) {
      const ds=todayString(); const [y,m,d]=ds.split('-').map(Number); window.AppState.statisticsDate=new Date(y,m-1,d,12);
    }
  }

  function periodLabel(){
    const r=getRange(), p=window.AppState.statisticsPeriod;
  ...
  function orderRows(income){
    return income.map((t,i)=>{
      ...
      const itemLines=items.length ? items.map(it=>{
        ...
        if(!unit && Array.isArray(window.AppState?.dishes)) { const d=window.AppState.dishes.find(x=>String(x.id)===String(it.dish_id)); if(typeof window.getDishCostFromDish==='function') unit=money(window.getDishCostFromDish(d)); }
  ...
  function renderShell(){
    ...
    page.innerHTML=`<div class="sp-wrap">
      <section class="sp-hero"><div><span>PHÂN TÍCH TÀI CHÍNH</span><h2>Thống kê & lợi nhuận</h2><p>Hiểu rõ doanh thu, vốn, phí nền tảng, chi phí và số tiền thực nhận.</p></div><div class="sp-hero-badge">📊</div></section>
      <section class="sp-control card"><div class="sp-tabs"><button data-period="day">Ngày</button><button data-period="week">Tuần</button><button data-period="month">Tháng</button></div><div class="sp-date"><div><label>Ngày tham chiếu</label><input id="spDate" type="date"></div><div class="sp-period-label" id="spPeriodLabel"></div><button id="spToday" type="button">Hôm nay</button></div></section>
      <section class="sp-kpi-grid">
        <article class="sp-kpi revenue"><span>DOANH THU</span><strong id="spRevenue">0 ₫</strong><small id="spOrders">0 đơn</small></article>
        <article class="sp-kpi cost"><span>GIÁ VỐN</span><strong id="spCost">0 ₫</strong><small id="spCostRate">0% doanh thu</small></article>
        <article class="sp-kpi fee"><span>PHÍ NỀN TẢNG</span><strong id="spFee">0 ₫</strong><small id="spFeeRate">Shopee + Grab</small></article>
        <article class="sp-kpi expense"><span>CHI PHÍ KHÁC</span><strong id="spExpense">0 ₫</strong><small id="spExpenseCount">0 khoản</small></article>
        <article class="sp-kpi profit"><span>LỢI NHUẬN THỰC</span><strong id="spProfit">0 ₫</strong><small id="spMargin">Biên lợi nhuận 0%</small></article>
      </section>
      <section class="sp-insight-grid"><article class="sp-insight" id="spInsightMain"></article><article class="sp-insight" id="spInsightPlatform"></article><article class="sp-insight" id="spInsightCost"></article></section>
      <section class="sp-section card"><div class="sp-section-head"><div><span>PHÂN TÍCH DÒNG TIỀN</span><h3>Doanh thu và chi phí theo ngày</h3><p>Nhìn nhanh những ngày bán tốt và ngày phát sinh chi phí cao.</p></div><div class="sp-legend"><i class="rev"></i>Thu <i class="exp"></i>Chi</div></div><div id="spChart" class="sp-chart"></div></section>
      <section class="sp-two-col"><article class="sp-section card"><div class="sp-section-head"><div><span>NGUỒN ĐƠN</span><h3>Hiệu quả theo nền tảng</h3><p>So sánh doanh thu, phí, vốn và thực nhận.</p></div></div><div id="spSources"></div></article><article class="sp-section card"><div class="sp-section-head"><div><span>CƠ CẤU</span><h3>Doanh thu phân bổ thế nào?</h3><p>Tỷ trọng vốn, phí và lợi nhuận trên doanh thu.</p></div></div><div id="spStructure"></div></article></section>
      <section class="sp-section card"><div class="sp-section-head"><div><span>MÓN BÁN</span><h3>Top món theo doanh thu và vốn</h3><p>Nhận biết món nào đóng góp nhiều tiền và món nào tốn vốn.</p></div></div><div id="spDishes"></div></section>
      <section class="sp-section card"><div class="sp-section-head"><div><span>DOANH THU</span><h3>Chi tiết từng đơn</h3><p>Hiểu chính xác: tổng tiền đơn → giá vốn → phí app → thực nhận.</p></div><strong id="spOrderCountLabel"></strong></div><div id="spOrdersList"></div></section>
      <section class="sp-two-col"><article class="sp-section card"><div class="sp-section-head"><div><span>CHI PHÍ</span><h3>Chi phí theo danh mục</h3><p>Khoản nào đang chiếm ngân sách nhiều nhất?</p></div></div><div id="spExpenseCats"></div></article><article class="sp-section card"><div class="sp-section-head"><div><span>CHI TIẾT</span><h3>Các khoản chi trong kỳ</h3><p>Danh sách để kiểm tra từng khoản.</p></div></div><div id="spExpenseList"></div></article></section>
      <section class="sp-bottom-note"><div>💡</div><div><b>Cách đọc lợi nhuận</b><span>Lợi nhuận thực = Doanh thu − Giá vốn − Chi phí nền tảng − Chi phí khác. Đơn Ngoài sàn không bị trừ phí app.</span></div></section>
    </div>`;
  }
  ...
  function renderDishes(income){
    ...
    const map={}; income.forEach(t=>{parseItems(t).forEach(it=>{const name=String(it.dish_name||'Món'); const qty=Math.max(1,Number(it.qty)||1); let unit=money(it.unit_cost); if(!unit && Array.isArray(window.AppState?.dishes)){const d=window.AppState.dishes.find(x=>String(x.id)===String(it.dish_id)); if(typeof window.getDishCostFromDish==='function')unit=money(window.getDishCostFromDish(d));} const revShare=qty ? (money(t.amount)/Math.max(1,parseItems(t).reduce((s,x)=>s+Math.max(1,Number(x.qty)||1),0))*qty) : 0; map[name] ||= {qty:0,revenue:0,cost:0};map[name].qty+=qty;map[name].revenue+=revShare;map[name].cost+=unit*qty;});});
  ...
  function render(){
    ...
    const dateInput=document.getElementById('spDate'); if(dateInput){ const d=window.AppState.statisticsDate; dateInput.value=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; dateInput.onchange=()=>{const ds=dateInput.value;const [y,m,d]=ds.split('-').map(Number);window.AppState.statisticsDate=new Date(y,m-1,d,12);render();}; }
    document.querySelectorAll('.sp-tabs button').forEach(b=>{b.classList.toggle('active',b.dataset.period===window.AppState.statisticsPeriod);b.onclick=()=>{window.AppState.statisticsPeriod=b.dataset.period;render();};});
    const today=document.getElementById('spToday');if(today)today.onclick=()=>{const ds=todayString();const [y,m,d]=ds.split('-').map(Number);window.AppState.statisticsDate=new Date(y,m-1,d,12);render();};
  ...
  window.statisticsPrevious = function(){ensureState();const d=window.AppState.statisticsDate;const p=window.AppState.statisticsPeriod;if(p==='day')d.setDate(d.getDate()-1);else if(p==='week')d.setDate(d.getDate()-7);else d.setMonth(d.getMonth()-1);render();};
  window.statisticsNext = function(){ensureState();const d=window.AppState.statisticsDate;const p=window.AppState.statisticsPeriod;if(p==='day')d.setDate(d.getDate()+1);else if(p==='week')d.setDate(d.getDate()+7);else d.setMonth(d.getMonth()+1);render();};
  window.statisticsToday = function(){ensureState();const ds=todayString();const [y,m,d]=ds.split('-').map(Number);window.AppState.statisticsDate=new Date(y,m-1,d,12);render();};
  window.setStatisticsPeriod = function(p){if(!['day','week','month'].includes(p))p='day';window.AppState.statisticsPeriod=p;render();};
