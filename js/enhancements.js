...
(function () {
    ...
    function syncSelectedItemsFromUI() {
        // QUAN TRỌNG: chỉ đồng bộ danh sách đang hiển thị, không được ghi đè
        // các món đã chọn ở danh mục khác. Đây là nguyên nhân khiến khi
        // chuyển từ Mì Trộn sang Cá Viên, phần Mì bị mất khỏi giá vốn lúc lưu.
        const boxes = document.querySelectorAll("#transactionDishMulti input[data-dish-id]");
        const dishMap = new Map((AppState.dishes || []).map(d => [String(d.id), d]));
        const selectedMap = new Map(
            (AppState.selectedOrderItems || []).map(item => [String(item.dish_id), { ...item }])
        );

        boxes.forEach(box => {
            const id = String(box.dataset.dishId);
            const dish = dishMap.get(id);
            if (!dish) return;

            const row = box.closest(".order-dish-card, .order-dish-row");
            const qtyEl = row?.querySelector("input[data-qty]");
            const qty = Math.max(1, parseInt(qtyEl?.value || selectedMap.get(id)?.qty || "1", 10) || 1);
            ...
            if (box.checked) {
                ...
                // Chỉ xóa món nằm trong danh mục hiện tại; các danh mục khác
                // vẫn giữ nguyên trong selectedOrderItems.
                selectedMap.delete(id);
            }
        });

        AppState.selectedOrderItems = [...selectedMap.values()];
    }
    ...
    window.saveTransaction = async function saveTransactionEnhanced() {
        ...
        // Store the exact local timestamp inside the order metadata so history/statistics
        // can always show date + time even when the DB only exposes a date column.
        let previousRecordedAt = "";
        if (AppState.editingTransactionId) {
            const oldTx = (AppState.transactions || []).find(t => String(t.id) === String(AppState.editingTransactionId));
            if (oldTx?.note && String(oldTx.note).startsWith(ORDER_MARKER)) {
                try { previousRecordedAt = JSON.parse(String(oldTx.note).slice(ORDER_MARKER.length))?.recorded_at || ""; } catch {}
            }
        }
        const notePayload = {
            note: userNote,
            items,
            version: 2,
            recorded_at: previousRecordedAt || new Date().toISOString()
        };
...
(function () {
    ...
    function renderSelectedOrderItems() {
        ...
        panel.innerHTML = `
            <div class="selected-order-header"><strong><span class="order-panel-icon">＋</span>Món trong đơn</strong><span>${items.reduce((s,x)=>s+(Number(x.qty)||1),0)} phần</span></div>
            <div class="selected-order-list">${items.map((x,i)=>`
                <div class="selected-order-row">
                    <div class="selected-order-info"><strong>${escapeHTML(x.dish_name || "Món")}</strong><small>${escapeHTML(x.category_name || "")}</small></div>
                    <div class="selected-order-qty" title="Nhập số lượng rồi nhấn Enter để xác nhận">
                        <span>SL</span>
                        <input type="number" inputmode="numeric" pattern="[0-9]*" min="1" step="1" value="${Math.max(1,Number(x.qty)||1)}" data-order-index="${i}" aria-label="Số lượng ${escapeHTML(x.dish_name || "món")}" onchange="setSelectedOrderQty(${i},this.value)">
                    </div>
                    <strong class="selected-order-cost">${formatMoney((Number(x.unit_cost)||0)*(Math.max(1,Number(x.qty)||1)))}</strong>
                    <button type="button" class="selected-order-remove" aria-label="Xóa ${escapeHTML(x.dish_name || "món")}" onclick="removeSelectedOrderItem(${i})">×</button>
                </div>`).join("")}</div>`;
    }
    ...
    function commitOrderQtyInput(input){
        if(!input) return;
        const idx = Number(input.dataset.orderIndex);
        if(Number.isInteger(idx) && AppState.selectedOrderItems?.[idx]){
            const value = Math.max(1, parseInt(input.value || "1", 10) || 1);
            input.value = String(value);
            AppState.selectedOrderItems[idx].qty = value;
            renderSelectedOrderItems();
            renderTransactionDishes();
            return true;
        }
    ...
    function commitPickerQtyInput(input){
        const row = input?.closest?.('.order-dish-card');
        const cb = row?.querySelector?.('input[data-dish-id]');
        if(!row || !cb) return false;
        const id = String(cb.dataset.dishId);
        ...
        map.set(id, {
            ...
            category_name: (AppState.categories || []).find(c => String(c.id) === String(dish.category_id))?.name || "",
            qty: value,
            unit_cost: typeof getDishCostFromDish === 'function' ? getDishCostFromDish(dish) : 0
        });
        cb.checked = true;
        AppState.selectedOrderItems = [...map.values()];
        renderSelectedOrderItems();
    ...
    document.addEventListener('keydown', function(e){
        ...
        if(target.matches?.('.selected-order-qty input[data-order-index]')){
            e.preventDefault();
            commitOrderQtyInput(target);
            return;
        }
        if(target.matches?.('#transactionDishMulti input[data-qty]')){
            e.preventDefault();
            commitPickerQtyInput(target);
        }
    });
...
/* =========================================================
   FINAL SOURCE + FEE UX PATCH
========================================================= */
(function(){
    const SOURCE_IDS = {
        ShopeeFood: "sourceShopee",
        GrabFood: "sourceGrab",
        "Ngoài sàn": "sourceOutside"
    };

    function isIncome(){ return String(AppState.transactionType || "thu").toLowerCase() === "thu"; }
    function feeRequired(){ return isIncome() && ["ShopeeFood","GrabFood"].includes(AppState.orderSource); }
    function getFee(){
        const el = document.getElementById("appFee");
        const n = Number(el?.value);
        return Number.isFinite(n) && n > 0 ? n : 0;
    }
    function saveButton(){
        return document.querySelector('#homePage button.primary-button[onclick*="saveTransaction"]');
    }

    function updateSourceUI(){
        const sourceBox = document.getElementById("orderSourceBox");
        const feeBox = document.getElementById("appFeeBox");
        const feeEl = document.getElementById("appFee");
        const button = saveButton();
        const income = isIncome();
        const outside = AppState.orderSource === "Ngoài sàn";
        const required = feeRequired();
        const valid = !income || outside || !required || getFee() > 0;

        if(sourceBox) sourceBox.style.display = income ? "block" : "none";
        if(feeBox) feeBox.style.display = income && !outside ? "block" : "none";
        if(feeEl){
            feeEl.required = required;
            feeEl.setAttribute("aria-required", String(required));
            if(outside) feeEl.value = "";
        }
        if(button){
            button.disabled = !valid;
            button.classList.toggle("is-disabled", !valid);
            button.setAttribute("aria-disabled", String(!valid));
            button.title = !valid ? "Nhập chiết khấu ShopeeFood/GrabFood trước khi lưu" : "Lưu giao dịch";
        }

        Object.entries(SOURCE_IDS).forEach(([source,id])=>{
            const el = document.getElementById(id);
            if(!el) return;
            const active = income && AppState.orderSource === source;
            el.classList.toggle("active", active);
            el.setAttribute("aria-pressed", String(active));
            el.tabIndex = 0;
        });

        const hint = document.getElementById("sourceFeeHint");
        if(hint){
            if(!income){ hint.textContent = ""; }
            else if(outside){ hint.textContent = "Đơn ngoài sàn: không khấu trừ phí nền tảng."; }
            else if(!getFee()){ hint.textContent = "Nhập số tiền chiết khấu để bật nút Lưu giao dịch."; }
            else { hint.textContent = "Phí nền tảng được trừ 1 lần cho toàn bộ đơn."; }
        }
    }

    window.setOrderSource = function(source){
        const allowed = ["ShopeeFood","GrabFood","Ngoài sàn"];
        AppState.orderSource = allowed.includes(source) ? source : "ShopeeFood";
        updateSourceUI();
    };

    function bindSources(){
        const box = document.getElementById("orderSourceBox");
        if(!box) return;
        box.querySelectorAll(".source-button").forEach(btn=>{
            ...
            btn.addEventListener("click", function(e){
                ...
                e.stopPropagation();
                const id = this.id;
                const source = Object.keys(SOURCE_IDS).find(k => SOURCE_IDS[k] === id);
                if(source) window.setOrderSource(source);
            }, { passive:false });
        ...
        const fee = document.getElementById("appFee");
        fee?.addEventListener("input", updateSourceUI);
        fee?.addEventListener("change", updateSourceUI);
        updateSourceUI();
    }

    const oldSetTypeFinal = window.setTransactionType;
    window.setTransactionType = function(type){
        if(oldSetTypeFinal) oldSetTypeFinal(type);
        if(String(type).toLowerCase() === "chi") AppState.orderSource = "Ngoài sàn";
        setTimeout(updateSourceUI, 0);
    };

    const oldRenderHomeFinal = window.renderHome;
    window.renderHome = function(){
        if(oldRenderHomeFinal) oldRenderHomeFinal();
        setTimeout(updateSourceUI, 0);
    };

    const oldClearFinal = window.clearTransactionForm;
    window.clearTransactionForm = function(){
        if(oldClearFinal) oldClearFinal();
        AppState.orderSource = "ShopeeFood";
        setTimeout(updateSourceUI, 0);
    };

    document.addEventListener("DOMContentLoaded", bindSources);
    setTimeout(bindSources, 0);
})();
